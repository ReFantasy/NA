    ...
    while True:
        ...
        while True:
            ...
            if jnp.linalg.norm(gk) <= epsilon:
                ...
                betak = jnp.dot(dk_old, jnp.dot(A, gk)) / jnp.dot(dk_old, jnp.dot(A, dk_old))
                # betak = jnp.dot(gk, gk) / jnp.dot(gk_old, gk_old)  # [optional] Fletcher-Reeves公式
                # betak = jnp.dot(gk, gk - gk_old) / jnp.dot(gk_old, gk_old)  # [optional] Polak-Ribiere公式
...
if __name__ == "__main__":
    ...
    test_groups = [
        ("第1组", jnp.array([2.0, 4.0])),
        ("第2组", jnp.array([-3.0, 5.0])),
        ("第3组", jnp.array([10.0, -10.0])),
        ("第4组", jnp.array([1.0, -1.0])),
    ]

    print(f"{'组别':<6}{'x0':<20}{'xstar':<28}{'fstar':>14}{'k':>6}")
    print("-" * 74)
    for group_name, x0 in test_groups:
        xstar, fstar, k = conjugate_gradient(
            objfun,
            x0,
            epsilon,
