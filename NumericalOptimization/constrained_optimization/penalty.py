...
from NumericalOptimization.gradient_methods import quasi_newton, newton_goldstein, gradient_descent
...
class InteriorPenalty:
    ...
    def optimize(self, verbose=True):
        ...
        while True:
            objfun = lambda x: self.objfun(x) + self.sigma * self.penalty_function(x)
            xstar, fstar, _ = newton_goldstein(
                objfun=objfun,
                x0=self.x0,
                epsilon=self.epsilon2,
                line_search_function=self.line_search_function,
...
if __name__ == "__main__":
    ...
    # ineq_cons = [g1]
    # x0 = jnp.array([3.0, 4.0])
    # inner_pen = InteriorPenalty(
    #     objfun, ineq_cons, x0, line_search_function=utils.LineSearchFunction(), epsilon=1e-6, epsilon2=0.001
    # )
    # xstar, fstar, k = inner_pen.optimize()
    # print(xstar, fstar, k)
